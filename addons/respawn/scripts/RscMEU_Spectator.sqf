...
_fnc_units = {
	...
	_units = [];
	{
		if (group _x in _groups) then {	if (!(_x getVariable ["MEU_Respawn_IsDead", false])) then { _units set [count _units,_x]; }; };
	} foreach _allunits;
...
switch _mode do {
	...
	case "init": {
		...
		//Spectator help
		("MEU_SpectatorHelpWindow" call BIS_fnc_rscLayer) cutRsc ["MEU_SpectatorHelp", "PLAIN"];
		MEU_Medical_SpectatorHelp = true;
		...
		//--- Create camera
		_pos = missionNamespace getVariable ["MEU_Medical_DeathPos", [0,0,0]];
		...
		if (count _units == 0) then {
			...
			_camera setpos (missionNamespace getVariable ["MEU_Medical_DeathPos", [0,0,0]]);
		...
		with missionnamespace do {
			...
			RscSpectator_player = if (count _units > 0) then {_units select 0} else {objnull};
			[] spawn {
				with uinamespace do {
	...
	case "keyDown": {
		...
		switch _key do {
			...
			case (DIK_SPACE): {
				...
				if ((!RscSpectator_map) && ((count _aliveUnits) != 0)) then {
			...
			case (DIK_H): {
				if (MEU_Medical_SpectatorHelp) then {
					MEU_Medical_SpectatorHelp = false;
					("MEU_SpectatorHelpWindow" call BIS_fnc_rscLayer) cutText ["", "PLAIN"];
				} else {
					MEU_Medical_SpectatorHelp = true;
					("MEU_SpectatorHelpWindow" call BIS_fnc_rscLayer) cutRsc ["MEU_SpectatorHelp", "PLAIN"];
				};
