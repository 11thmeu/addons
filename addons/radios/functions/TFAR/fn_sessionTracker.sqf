...
// DELETE IT COZ BETTER PERFOMANCE
/*[] spawn {
	...
	if (isMultiplayer) then {
		sleep (60 * 1);
		 while {true} do {
			_request = format['TRACK	piwik.php?idsite=2&rec=1&url="radio.task-force.ru&action_name=%1&rand=%2&uid=%3&cvar={"1":["group","%4"], "2":["playableUnits","%5"], "3":["allUnits","%6"], "4":["BIS_fnc_listCuratorPlayers","%7"], "5":["playerSide","%8"], "5":["faction","%9"], "6":["isServer","%10"], "7":["isDedicated","%11"], "8":["missionName","%12"], "9":["currentSW","%13"], "10":["currentLR","%14"], "11":["nearPlayers","%15"], "12":["farPlayers","%16"], "13":["typeof","%17"], "14":["diag_fps","%18"], "15":["diag_fpsmin","%19"], "16":["daytime","%20"], "17":["vehicle","%21"], "18":["time","%22"], "19":["version","%23"]}', _action, random 1000, getPlayerUID player, count (units group player), count playableUnits, count allUnits, count (call BIS_fnc_listCuratorPlayers), playerSide, faction player, isServer, isDedicated, missionName, count (currentUnit call TFAR_fnc_radiosList), count (currentUnit call TFAR_fnc_lrRadiosList), count tf_nearPlayers, count tf_farPlayers, typeof currentUnit, diag_fps, diag_fpsmin, daytime, typeof (vehicle currentUnit), time, TF_ADDON_VERSION];
